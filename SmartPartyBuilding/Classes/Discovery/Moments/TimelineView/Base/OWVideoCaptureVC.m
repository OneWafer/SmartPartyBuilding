...
@interface OWVideoCaptureVC ()<AVCaptureFileOutputRecordingDelegate>
...
@implementation OWVideoCaptureVC
...
-(void)captureOutput:(AVCaptureFileOutput *)captureOutput didFinishRecordingToOutputFileAtURL:(NSURL *)outputFileURL fromConnections:(NSArray *)connections error:(NSError *)error
{
    ...
    wh_Log(@"文件路径: %@", outputFileURL);
    
    NSData *data = [NSData dataWithContentsOfURL:outputFileURL];
    wh_Log(@"文件大小: %luk", (unsigned long)([data length]/1024));
    ...
    [avAssetExportSession exportAsynchronouslyWithCompletionHandler:^(void){
        switch (avAssetExportSession.status) {
            case AVAssetExportSessionStatusFailed:
                wh_Log(@"处理失败 %@",[avAssetExportSession error]);
                break;
...
-(UIImage *) generateScreenshot:(NSURL *) url
{
    ...
    if(error){
        wh_Log(@"截取视频缩略图时发生错误，错误信息：%@",error.localizedDescription);
        return nil;
    }
