...
@interface OWPlainGridImageView ()

@property (nonatomic, strong) NSMutableArray *imageViews;

@end

@implementation OWPlainGridImageView
...
-(void)layoutSubviews
{
    ...
    for (int row=0; row<3; row++) {
        for (int column=0; column<3; column++) {
            ...
            x = (width+Padding)*column;
            y = (height+Padding)*row;
            OWImageUnitView *imageUnitView = [_imageViews objectAtIndex:(row*3+column)];
...
-(void) onLongPress:(UILongPressGestureRecognizer *) recognizer
{
    if (recognizer.state == UIGestureRecognizerStateBegan){
        OWImageUnitView *view = (OWImageUnitView *)recognizer.view;
