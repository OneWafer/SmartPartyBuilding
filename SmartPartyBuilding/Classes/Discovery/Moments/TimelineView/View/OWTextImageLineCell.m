...
@interface OWTextImageLineCell ()

@property (strong, nonatomic) MLLinkLabel *textContentLabel;

@end

@implementation OWTextImageLineCell
...
- (void) initCell
{
    ...
    if (_gridImageView == nil) {
        ...
        _gridImageView = [[OWGridImageView alloc] initWithFrame:CGRectMake(x, y, width, height)];
...
- (void)updateWithItem:(OWTextImageLineItem *)item
{
    ...
    CGFloat gridHeight = [OWGridImageView getHeight:item.thumbImages maxWidth:GridMaxWidth oneImageWidth:item.width oneImageHeight:item.height];
...
- (CGFloat)getCellHeight:(OWTextImageLineItem *)item
{
