...
@interface OWBaseLineCell ()<OWLikeCommentViewDelegate, OWLikeCommentToolbarDelegate>

@property (nonatomic, strong) OWBaseLineItem *item;
...
@property (nonatomic, strong) OWLikeCommentView *likeCommentView;


@property (nonatomic, strong) OWLikeCommentToolbar *likeCommentToolbar;
...
- (void) initBaseCell
{
    ...
    if (_likeCommentView == nil) {
        ...
        width = BodyMaxWidth;
        height = 10;
        _likeCommentView = [[OWLikeCommentView alloc] initWithFrame:CGRectMake(x, y, width, height)];
...
- (void) updateLocationLikeComment:(CGFloat)height
{
    ...
    if (self.item.likes.count ==0 && self.item.comments.count == 0) {
        ...
        height = [OWLikeCommentView getHeight:self.item maxWidth:BodyMaxWidth];
...
- (CGFloat)getCellHeight:(OWBaseLineItem *)item
{
    ...
    if (!(item.likes.count == 0 && item.comments.count == 0)) {
        height+=[OWLikeCommentView getHeight:item maxWidth:BodyMaxWidth]+LikeCommentTimeSpace;
    }
...
- (OWBaseLineCell *)getCell:(UITableView *)tableView
{
    return nil;
}
