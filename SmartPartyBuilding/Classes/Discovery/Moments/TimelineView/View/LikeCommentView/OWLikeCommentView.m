...
@implementation OWLikeCommentView
...
-(void) initView
{
    ...
    if (_likeLabel == nil) {
        ...
        __block OWLikeCommentView *likeCommentView = self;
...
-(void)updateWithItem:(OWBaseLineItem *)item
{
    ...
    if (item.comments.count > 0) {
        ...
        for (int i=0; i<labelCount; i++) {
            OWLinkClickLabel *label = [_commentLabels objectAtIndex:i];
        ...
        for (int i=0;i<item.comments.count;i++) {
            ...
            OWLineCommentItem *commentItem = [item.comments objectAtIndex:i];
...
-(OWLinkClickLabel *) createLinkLabel
{
    
    OWLinkClickLabel *lable = [[OWLinkClickLabel alloc] initWithFrame:CGRectZero];
