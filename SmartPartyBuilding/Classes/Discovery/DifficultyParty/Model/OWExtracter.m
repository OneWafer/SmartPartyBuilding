...
+ (OWExtracter *)parseEntity:(id)entity keyPath:(NSString *)keyPath {
    ...
    OWExtracter *parseModel = [OWExtracter new];
...
- (void)_parseEntity:(id)entity keyPath:(NSString *)keyPath {
    ...
    switch (order) {
        ...
        case ExtracterOrderExtract: {
            ...
            NSMutableArray<OWExtracter *> *contents = [NSMutableArray new];
            ...
            for (id object in array) {
                ...
                OWExtracter *extrancter = [[OWExtracter alloc] initWithValue:value];
...
- (BOOL)isRationalKeyPath:(NSString *)keyPath inEntity:(id)entity {
    ...
    if (![keyPath hasPrefix:@"."] && ![keyPath hasPrefix:@"->"]) {
        wh_Log(@"Extracter keyPath解析错误, 必须以操作指令.或->开头");
        return NO;
    }
            ...
            switch (order) {
                case ExtracterOrderInto: {
                    if ([entity isKindOfClass:[NSArray class]]) {
                        wh_Log(@"Extracter 数据解析错误 , -> 指令不能作用于数组");
                        return NO;
                    }
                    
                    id value = [entity valueForKey:key];
                    if (!value) {
                        wh_Log(@"Extracter 数据解析错误 , 不存在该key: %@", key);
                        return NO;
                    } else {
                        arrayKeyName = key;
                        entity = value;
                ...
                case ExtracterOrderExtract: {
                    ...
                    if (![array isKindOfClass:[NSArray class]]) {
                        wh_Log(@"Extracter 数据解析错误 , .%@ 指令的对象不为数组", key);
                        return NO;
                    }
                    
                    if (array.count <= 0) {
                        wh_Log(@"Extracter 数据解析错误 , %@ 数组为空无法提取 %@ 的值", arrayKeyName, key);
                        return NO;
                    }
                    ...
                    if (!value) {
                        ...
                        wh_Log(@"Extracter 数据解析错误 , key: %@ 类型不是NSString或者NSNumber", key);
                        return NO;
                    }
