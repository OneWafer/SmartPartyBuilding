...
@implementation OWExcPartyMemCell

static NSString *const identifier = @"OWExcPartyMemCell";
+ (instancetype)cellWithTableView:(UITableView *)tableView
{
    OWExcPartyMemCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (cell == nil) {
        cell = [[OWExcPartyMemCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
...
- (void)setMember:(OWExcPartyMember *)member
{
    _member = member;
    [self.titleImgView sd_setImageWithURL:[NSURL URLWithString:member.avatar] placeholderImage:wh_imageNamed(@"home_news_place")];
    self.titleLabel.text = member.title;
    self.timeLabel.text = member.selectionDate;
}
